...
<div class="container-fluid py-4">
    <div class="card tambah-card p-4">
        <div class="d-flex align-items-center mb-4">
            ...
            <h4 class="fw-bold m-0 text-dark">Edit Alat</h4>
        </div>

        <form action="<?= base_url('inventaris/update/' . $alat['id']) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
                ...
                <div class="col-lg-7">
                    <div class="p-3 bg-light rounded-4 border-0">
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama_alat" class="form-control shadow-sm" value="<?= esc($alat['nama_alat']) ?>" placeholder="Masukkan nama alat..." required>
                        </div>

                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label class="form-label-custom">Jumlah (Stok)</label>
                                <input type="number" name="jumlah" class="form-control shadow-sm" min="1" value="<?= esc($alat['jumlah']) ?>" required>
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="kategori" class="form-select shadow-sm" required>
                                    ...
                                    <?php if (!empty($kategori_list)): ?>
                                        <?php foreach ($kategori_list as $kat): ?>
                                            <option value="<?= esc($kat['nama_kategori']) ?>" <?= ($alat['kategori'] == $kat['nama_kategori']) ? 'selected' : '' ?>>
                                                <?= esc($kat['nama_kategori']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <option value="Elektronika" <?= ($alat['kategori'] == 'Elektronika') ? 'selected' : '' ?>>Elektronika</option>
                                        <option value="Mekanik" <?= ($alat['kategori'] == 'Mekanik') ? 'selected' : '' ?>>Mekanik</option>
                                    <?php endif; ?>
                                </select>
                            </div>
                        </div>
